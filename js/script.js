...
function myForms() {
	...
	const textErrorDefault = "Minimum 2 letters";
	const textErrorEmail = "Email entered incorrectly";
	const textErrorPassword = "Minimum 8 letters, no characters like \"./,%;\" etc.";
	const textErrorTextarea = "Minimum 2 letters";
	const textErrorCheckbox = "This is a required field";
	const textErrorPhone = "Number entered incorrectly";
						...
						if (!elementTarget.classList.contains("_invalid")) {
							...
							if (elementTarget.classList.contains("_password")) {
								...
								if (minimum8Chars.test(input.value) && beginWithoutDigit.test(input.value) && withoutSpecialChars.test(input.value) && containsLetters.test(input.value)) {
									...
									input.classList.add("_invalid");
									let textInfo = `${textErrorPassword}`;
									formBlur(textInfo, e)
								}
							}
					...
					textarea.addEventListener("blur", function (e) {
						...
						if (!elementTarget.classList.contains("_invalid")) {
							...
							if (textarea.value.length < 2) {
								...
								let textInfo = `${textErrorTextarea}`;
								formBlur(textInfo, e)
			...
			function validCheckbox(checkbox) {
				if (checkbox.classList.contains("_required")) {
					if (checkbox.checked === true) {
						...
						error++;
						let textInfo = `${textErrorCheckbox}`;
			...
			function formValid(e) {
				inputs.forEach(input => {
					if (input.classList.contains("_required")) {
						...
						if (!input.classList.contains("_email") && !input.classList.contains("_password") && !input.classList.contains("_phone")) {
							let textInfo = `${textErrorDefault}`;
							addError(textInfo)
						}
